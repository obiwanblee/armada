...
dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/mesa/mesa-*.fc44.armada.*.rpm

# Patched mangohud: Adreno GPU load/clock/temp for mainline drm/msm (msm_dpu).
dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/mangohud/mangohud-*.fc44.armada.*.rpm
...
dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/inputplumber/inputplumber-*.rpm

# Patched NetworkManager: /etc/NetworkManager/ignore-sleep keeps wifi up across fake-suspend.
dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/networkmanager/*.rpm

dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/armada-splash/*.rpm
...
dnf5 -y install --skip-unavailable --setopt=install_weak_deps=False /packages/fex/fex-emu-*.rpm
